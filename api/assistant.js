import { ASSISTANT_PROMPT, buildAssistantUserPrompt } from './_prompts/assistant.js'
...
export default async function handler(req, res) {
  ...
  const { situation, question } = req.body || {}

  if (!situation || !question) {
    return res.status(400).json({ success: false, error: 'Missing situation or question' })
  }
    ...
    const aiRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: buildAssistantUserPrompt(situation, question) },
        ],
