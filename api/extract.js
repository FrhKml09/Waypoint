import { EXTRACT_PROMPT, buildExtractUserPrompt } from './_prompts/extract.js'
...
export default async function handler(req, res) {
  ...
  const { message, author, trip } = req.body || {}

  if (!message || !trip) {
    return res.status(400).json({ success: false, error: 'Missing message or trip' })
  }
    ...
    const aiRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: buildExtractUserPrompt(message, author, trip) },
        ],
