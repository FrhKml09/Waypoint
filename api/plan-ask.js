import { PLAN_ASK_PROMPT, buildPlanAskUserPrompt } from './_prompts/planAsk.js'
...
export default async function handler(req, res) {
  ...
  const { trip, itinerary, disrupted, question } = req.body || {}

  if (!trip || !itinerary || !question) {
    return res.status(400).json({ success: false, error: 'Missing trip, itinerary, or question' })
  }
  ...
  try {
    const aiRes = await fetch(url, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: 'system', content: PLAN_ASK_PROMPT },
          { role: 'user', content: buildPlanAskUserPrompt(trip, itinerary, disrupted, question) },
        ],
