import { PLAN_EXTRACT_PROMPT, buildPlanExtractUserPrompt } from './_prompts/planExtract.js'
...
export default async function handler(req, res) {
  ...
  const { message, author, trip, catalog } = req.body || {}

  if (!message || !trip || !catalog) {
    return res.status(400).json({ success: false, error: 'Missing message, trip, or catalog' })
  }
  ...
  try {
    const aiRes = await fetch(url, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: 'system', content: PLAN_EXTRACT_PROMPT },
          { role: 'user', content: buildPlanExtractUserPrompt(message, author, trip, catalog) },
        ],
