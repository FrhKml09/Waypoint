import { REPLAN_PROMPT, buildReplanUserPrompt } from './_prompts/replan.js'
...
export default async function handler(req, res) {
  ...
  const { trip, reason, day } = req.body || {}

  if (!trip || !reason) {
    return res.status(400).json({ success: false, error: 'Missing trip or reason' })
  }
    ...
    const aiRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: buildReplanUserPrompt(trip, reason, day) },
        ],
